...
object SparkMapReduce {
  ...
  def runMapReduce(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = 
  {
    ...
    //println("Hello World!")
    val mapper = joinRdd.map(row => (row(0).toString, 1))
    val reducer = mapper.reduceByKey((key,value) => key+value).sortBy(row=>row._2)
    var result = reducer.map(mapper => (mapper._2)).toDF().coalesce(1)
